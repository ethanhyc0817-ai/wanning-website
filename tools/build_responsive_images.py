...
MIN_DISPLAY_WIDTH = 200   # below this an image is a logo/icon; see note in process_html
AVIF_Q, WEBP_Q = 55, 72
...
# Wrapping an <img> in <picture> inserts a box between the image and its parent.
# Any layout that assumes the <img> is a direct child then silently breaks — on
# this site `.gallery-strip` is display:flex with `flex: 0 0 88%` set on the img,
# so the strip collapsed from 239px to 62px on mobile. `display: contents` makes
# <picture> generate no box, so the <img> stays the flex/grid item it was before.
PICTURE_CSS = 'picture{display:contents}picture>source{display:none}'
# The `picture>source{display:none}` half is not optional: with display:contents
# the <source> children also participate in the parent's layout, so in a grid
# each picture contributed 3 items instead of 1 and the gallery strip rendered
...
def ensure_picture_css(html):
    ...
    if i == -1:
        i = html.find('</head>')
        return html[:i] + f'<style>{PICTURE_CSS}</style>\n' + html[i:] if i != -1 else html
    j = i + len('<style>')
    return html[:j] + '\n' + PICTURE_CSS + '\n' + html[j:]
...
def process_html(path):
    ...
    for m in IMG_TAG.finditer(html):
        ...
        # Skip logos/icons. `sizes` does not only choose the file — for an image
        # with no CSS width (e.g. `max-width:148px; height:auto`) it also sets the
        # density-corrected intrinsic size, so it changes how big the image draws.
        # This tool stores one `sizes` per image path, but a logo is reused at
        # different sizes on different pages, so a single value is wrong somewhere:
        # the footer PerfectSwell mark shrank from 148px to 47px. Small images save
        # little anyway, so leave them alone rather than guess.
        ...
        # Deliberately do NOT inject width/height attributes.
        #
        # It looks like free CLS protection, but most of this site's images are
        # styled with a CSS `width` and no `height` (cards, tiles, hero strips).
        # In that case the height attribute acts as a presentational hint, so an
        # <img width="2560" height="1709"> in a 342px-wide slot lays out as
        # 342x1709 and object-fit:cover crops into a hugely zoomed sliver.
        # That shipped once and had to be reverted — leave sizing to the CSS.
        newtag = tag
...
def build_css_backgrounds(paths):
    ...
    for path in paths:
        ...
        for src in sorted(set(CSS_URL.findall(html))):
            ...
            remap[src] = url_quote('/' + out_rel) if src.startswith('/') else url_quote(out_rel)
            converted += 1
        # Rewrite ONLY inside url(...). A plain string replace also hits <img src>
        # and srcset entries that happen to reference the same file, which would
        # swap the JPEG fallback out from under browsers that need it.
        def _sub(m):
            return m.group(0).replace(m.group(1), remap[m.group(1)]) if m.group(1) in remap else m.group(0)
        html = CSS_URL.sub(_sub, html)
